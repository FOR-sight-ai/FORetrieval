...
# OpenAI / OpenRouter / Ollama (OpenAI-compatible API)
from pydantic_ai.models.openai import OpenAIModel
from pydantic_ai.providers.openai import OpenAIProvider

# Mistral (native PydanticAI client)
try:
...
# Language & file parsers (optional)
try:
...
def _iso_utc(ts: float) -> str:
    return (
        datetime.fromtimestamp(ts, tz=timezone.utc).replace(microsecond=0).isoformat()
    )
...
def _build_model_from_cfg(ai_cfg: Dict[str, Any]):
    """
    Dynamically builds a PydanticAI model according to the chosen provider.

    ai_cfg expects for example:
      {
        "provider": "openai" | "openrouter" | "mistral" | "mistralai" | "ollama",
        "api_key": "...",
        "base_url": "...",     # optional
        "name": "model-id"     # optional
      }
    """
    ...
    if provider_raw in ("mistral", "mistralai"):
        if not _MISTRAL_AVAILABLE:
            raise RuntimeError(
                "The 'mistral' provider is requested but the Mistral dependencies for pydantic-ai are not installed.\n"
        ...
        if not name:
            # Important: namespace required by OpenRouter
        ...
        if not name:
            name = "llama3.1"  # adjust according to your local tags
        provider = OpenAIProvider(api_key=api_key, base_url=base_url)
        return OpenAIModel(model_name=name, provider=provider)
...
def ai_metadata_provider_factory(
    ...
    """
    Returns a callable(Path) -> Dict complete metadata.
    - Without ai_cfg => provider 'no-LLM' (fills only the raw fields).
    - With ai_cfg => builds a PydanticAI Agent with the right model according to the provider,
                     instructions=AI_SYSTEM_PROMPT, output_type=AIMetadata.
    """

    # ----- Without AI: just the "raw" fields -----
    if not ai_cfg:
    ...
    # ----- With AI: builds the model & PydanticAI agent -----
    model = _build_model_from_cfg(ai_cfg)

    # NB: PydanticAI canonical structure: instructions = system, output_type = Pydantic schema
    # Adding a "tools": [] parameter to avoid any tool-calling attempt if the backend doesn't support it
    agent = Agent(
        model=model,
    ...
    def _provider(p: Path) -> Dict[str, Any]:
        ...
        # User context (passed in the run prompt)
        ctx = {
            "file_name": p.name,
            "mime": mime,
            "ext": ext,
        ...
        # ✅ PydanticAI canonical API: run(prompt) -> result.output (typed output_type)
        try:
            result = agent.run_sync(
                user_prompt
            )  # pydantic-ai >= 0.0.15 (depending on versions)
        except AttributeError:
            result = asyncio.run(agent.run_sync(user_prompt))  # universal fallback
